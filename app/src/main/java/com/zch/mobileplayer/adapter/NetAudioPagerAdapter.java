...
public class NetAudioPagerAdapter extends BaseAdapter {
    ...
    private final List<NetAudioPagerData.ListEntity> mDataList;

    public NetAudioPagerAdapter(Context context, List<NetAudioPagerData.ListEntity> dataList) {
    ...
    public int getItemViewType(int position) {
        ...
        NetAudioPagerData.ListEntity listEntity = mDataList.get(position);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //绑定数据
        //根据位置得到数据,绑定数据
        NetAudioPagerData.ListEntity mediaItem = mDataList.get(position);
    ...
    private void bindData(int itemViewType, ViewHolder viewHolder, NetAudioPagerData.ListEntity mediaItem) {
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                ...
                viewHolder.tv_video_duration.setText(TimeUtils.str2Time(mediaItem.getVideo().getDuration() * 1000));

                break;
            case TYPE_IMAGE://图片
                bindData(viewHolder, mediaItem);
                viewHolder.iv_image_icon.setImageResource(R.drawable.item_bg);
...
//                    x.image().bind(viewHolder.iv_image_icon, mediaItem.getImage().getBig().get(0));
                    Glide.with(mContext).load(mediaItem.getImage().getBig().get(0)).placeholder(R.drawable.item_bg).error(R.drawable.item_bg).diskCacheStrategy(DiskCacheStrategy.ALL).into(viewHolder.iv_image_icon);
                }
                break;
            case TYPE_TEXT://文字
                bindData(viewHolder, mediaItem);
    ...
    private View initView(View convertView, int itemViewType, ViewHolder viewHolder) {
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                convertView = View.inflate(mContext, R.layout.item_all_video, null);
                //在这里实例化特有的
                ...
                break;
            case TYPE_IMAGE://图片
                convertView = View.inflate(mContext, R.layout.item_all_image, null);
                viewHolder.iv_image_icon = (ImageView) convertView.findViewById(R.id.iv_image_icon);
                break;
            case TYPE_TEXT://文字
                convertView = View.inflate(mContext, R.layout.item_all_text, null);
                break;
            case TYPE_GIF://gif
                convertView = View.inflate(mContext, R.layout.item_all_gif, null);
                viewHolder.iv_image_gif = (GifImageView) convertView.findViewById(R.id.iv_image_gif);
                break;
            case TYPE_AD://软件广告
                convertView = View.inflate(mContext, R.layout.item_all_ad, null);
